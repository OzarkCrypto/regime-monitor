...
.tl-bar{{display:flex;height:32px;border-radius:6px;overflow:hidden;margin-bottom:4px;border:1px solid var(--b)}}
.tl-seg{{position:relative;min-width:2px;display:flex;align-items:center;justify-content:center;font-family:var(--m);font-size:7px;font-weight:700;color:rgba(0,0,0,.4);overflow:hidden;cursor:default}}
...
app.innerHTML=h}}
function rTimeline(){{const tl=D.timeline;if(!tl||!tl.length)return'';const t0=new Date(tl[0].start).getTime();const t1=new Date(tl[tl.length-1].end).getTime();const span=t1-t0;let h='<div class="tl-bar">';tl.forEach(seg=>{{const s=new Date(seg.start).getTime();const e=new Date(seg.end).getTime();const w=Math.max(0.3,((e-s)/span)*100);const col=RC[seg.l]||'#a8a29e';const ic=seg===tl[tl.length-1];h+='<div class="tl-seg" style="width:'+w+'%;background:'+col+(ic?';box-shadow:inset 0 0 0 2px rgba(0,0,0,.3)':'')+'" title="'+seg.l+' ('+seg.start+' \\u2192 '+seg.end+')"></div>'}});h+='</div>';h+='<div class="tl-dates"><span>'+tl[0].start.substring(0,7)+'</span><span>'+tl[tl.length-1].end.substring(0,7)+'</span></div>';const seen=new Set();h+='<div class="tl-legend">';tl.forEach(seg=>{{if(!seen.has(seg.l)){{seen.add(seg.l);h+='<span><span class="dot" style="background:'+(RC[seg.l]||'#a8a29e')+'"></span>'+seg.l+'</span>'}}}});h+='</div>';return h}}
